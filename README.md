...
| `sg-design-humanize` | AI 티 디자인을 구조부터 재설계(페이지·섹션·컴포넌트). 개편이 기본 — 탐지 38룰 + 게이트 5종(같은 형태 재생산은 히스토리·팔레트 검사로 차단) + 벤치마크 실측 갭 — "조정만"은 시그니처 린트가, 텍스트·기능 불변은 게이트가 차단. 방향은 입력 유도 — 고정 스킨 없음 |
