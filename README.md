...
| `sg-web-replicate` | 레퍼런스 프론트를 첫 진입 팝업·저장상태·전 라우트·반응형·상호작용·모션까지 완전 복제. 깨끗한 context에서 팝업 닫기·오늘 하루 보지 않기·reload 미노출을 자동 계약 → query/status/redirect/canonical/renderer/404 원장 → 정적+정상 모션 before/mid/after 캡처 → `route × viewport × state` strict 집계. 임의 고정 날짜 없이 실제 관찰 시작 시각을 증거로 보존 |
