...
// facts 없이 시안을 뽑으면 **빈 틀 시안**이 나오고, 빌드는 그 틀을 베낀다(§0g).
// 실측: 이 사슬이 "하는 화면 12/24가 첫 화면의 10~20%만 일로 덮는" 결과의 상류였다.
// 시안은 빌드 산출물의 목표물이므로 콘텐츠 조달이 먼저다 — 지어내서 채우는 것은 금지.
const noFacts = targets.filter((p) => !p.facts?.length);
if (noFacts.length) {
  console.error(
    `🔴 facts 없는 페이지 ${noFacts.length}개 — 무엇이 들어가는지 모르는 화면의 시안은 빈 틀이 된다(§0g):\n  - ` +
    noFacts.map((p) => p.route).join("\n  - ") +
    `\n\n계약의 해당 페이지에 facts(이 화면에 실제로 들어갈 것 — 제품에서 전수 추출)를 채우고 다시 돌려라.`);
  exit(1);
}
...
## 이 화면에 실제로 들어갈 것 (계약 facts — 이걸로 화면을 채워라, 지어내지 마라)
...
- **글만 있는 화면 금지** — 제목+문단만 늘어놓지 말고 이 화면의 일(입력·항목·표·데이터)을 넣어라${page.type.verb === "do" ? `
- **소개문 도입 금지** — 아이브로>큰 제목>리드 문단으로 열지 마라. 제목은 한 줄이면 충분하고, 그 바로 아래에 이 화면의 일(위 facts의 실체)이 첫 화면의 주인공으로 온다. 실측: 하는 화면 9개가 정책 문서와 똑같은 도입부로 열려 전부 같은 템플릿으로 읽혔다(SAME-OPENING)` : ""}
