...
- **P0 계약** — **먼저 동사를 정한다**: 이 페이지에서 사용자가 읽는가·고르는가·쓰는가·정하는가·적는가·보는가. 동사가 유형을 결정한다(page-rules §0b). **소개문으로 열 수 있는 건 '읽는다'뿐이다**(랜딩·소개·문서). §0b 표에 없는 화면이면 **거기서 동사를 선언한다**(`read:`·`do:`) — 목록에 없다고 랜딩으로 회귀하는 것이 이 스킬 최대 사고다. 그다음 랜딩 tokens.json 위치와 이 페이지가 돕는 결정을 한 문장으로. 사실 재고는 프로젝트 문서·실제 제품에서 전수 추출한다(가격·플랜·제약은 지어내지 않는다).
