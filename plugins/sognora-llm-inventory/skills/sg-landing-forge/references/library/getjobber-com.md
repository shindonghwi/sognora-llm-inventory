...
- **체크리스트**
  - ① 통과 — **차단 해제 확인.** 직전 시도는 css 2장·자산 1건으로 봇 차단이 의심됐으나, 표준 UA가 붙은 계기로 재시도해 **css 161장 · 자산 96건 · @keyframes 7개**를 정상 수확했다. scrollHeight 6,968 / 11,903px(데스크톱 최단, 모바일은 1.7배 — 4열 업종 타일과 2열 카드가 세로로 풀린다). **백지 밴드 0%**, 챌린지·쿠키월 없음.
  - ② **통과** — IR 섹션 **10** vs 눈 9(차 1), 구성도 정확하다:
    - **`#0`이 진짜 히어로다** — y=113, h=690, 헤드라인 50px/900 "Run a stronger service business", **자산 슬롯 12개**(사진 모자이크). 초기 판본에서는 이 자리를 비가시 오버레이가 차지해 히어로가 통째로 사라져 있었다.
    - **그 오버레이는 `layers`로 이동** — ambient layers 1건 `NavigationOverlay_overlay__v_7bc@0`(`position:fixed`, `opacity:0`, `z-index:99989`). 섹션이 아니라 레이어로 분류된다는 설계가 정확히 작동한다.
    - 차이 1은 하단 리소스 영역(h=2,031)이 카드 묶음 3개로 갈린 것(`#6` "Masters of Home Service" / `#7` "Free Tools" / `#8` "Jobber Grants"). 각자 24px/900 제목을 가져 과분할치고는 온건하지만, 대신 이 영역의 상위 헤드라인 "Our business is helping yours succeed."(40px/900)가 어느 섹션에도 남지 않았다.
    - 사진 스트립(y=2,108, h=160)은 h≥200 하한 미달로 제외.
  - ③ 🟡 — @keyframes 7개인데 **명명된 고유 모션이 없다**: `animation-lb2s9x`×4, `animation-1rpgdk4`×2(emotion 자동 생성 해시) + `onetrust-fade-in`(서드파티). 교차 확인: **record.animations 0건** — 실행 중 애니메이션이 하나도 없다. 이 페이지의 모션은 전적으로 **CSS transition 기반**이며 duration 분포가 뒷받침한다(250ms 73건 · 300ms 17건 · 200ms 4건 · 600ms 2건 · 100ms 2건). CSS-in-JS(emotion)를 쓰면서도 모션을 keyframes로 만들지 않는다.
  - ④ **실패** — 자산 96건, **이미지 rect 결합률 24%(22/90)로 5개 중 최저.** 원인은 두 갈래다: (a) 지연 로드 이미지가 뷰포트 밖이라 rect 미결합, (b) **하단 리소스 카드 이미지가 실제로 깨져 있다** — 데스크톱·모바일 스크린샷 양쪽에서 `Masters of Home Service - A Jobber Podcast`, `Jobber Academy`, `Free Tools`, `Shows up when it matters`, `Adapts to your style`가 이미지 대신 alt 텍스트로 노출됐다. 수집 문제가 아니라 원본의 상태다.
  - ⑤ 부분 통과 — **hover 9건으로 5개 중 품질이 가장 좋다**(28~34프레임 실측 보간: 라임 `rgb(168,227,0)`→`rgb(185,233,51)`(밝게) 및 →`rgb(134,182,0)`(어둡게), 링크 `opacity 1→0.7`, 투명→라임 채우기). **reveal 0건.**
  - ⑥ 재현 불가 목록 아래 별항.
- **IR 요약(계기 최종본)** — 섹션 **10**(데스크톱)/**9**(모바일 — 중간 판본의 29에서 정상화됐다). **컨테이너 1,360px 단일** — 5개 중 유일하게 한 값만 쓴다. 타입 스케일 상위 3: 50px/900(1회, 히어로) · 40px/900(5회, 섹션 헤드라인) · 32px/900(3회). **weight 900이 스케일 전체를 지배**한다(24px/900이 17회, 18px/900이 7회). duration 후보 250ms(73건)·300ms(17건)·200ms(4건)·600ms(2건)·100ms(2건). @keyframes 7개(고유 명명 없음). 리스너 상위 3: error 2 · message 1 · unhandledrejection 1 — CDP 인벤토리가 사실상 비었다(리스너를 요소가 아닌 상위에 위임하는 구조로 보인다). 폰트 jobberPro(디스플레이 900) + inter(본문), 선언만 되고 안 쓰이는 handwritten·stencil. bgSequence 2종(`rgb(242,240,234)` 종이색, `rgb(1,41,57)` 딥네이비). **ambient layers 1건.**
- **시각 확인** — 눈으로 센 섹션 9개(히어로 네이비 / "all-in-one system" 종이 / 400,000 통계 네이비 / 사진 스트립 / "AI built for blue collar" / "Proud partner…50 industries" / 증언+CTA 네이비 / "Our business is helping yours succeed" 리소스 / 푸터 네이비).
  - **최대 시그니처: 찢어진 종이(deckle) 밴드 전환.** 네이비↔종이색 경계가 전부 직선이 아니라 **불규칙하게 찢긴 가장자리**다. 페이지 전체에서 5회 이상 반복되며 배경 교대 리듬 자체를 이 마스크가 담당한다. 모바일에서도 그대로 유지된다.
  - 그 외 실제로 본 시그니처: 상단 공지 바(이벤트 + 🔥 이모지) / 히어로 좌측 텍스트 + **우측 인물 사진 모자이크**(각 사진 하단에 `이름 / 상호` 캡션 오버레이) + 앱스토어·구글플레이 평점 2종 / 라임 `Start Free Trial` + 라임 아웃라인 `Find Your Plan` 2버튼 대비 / 탭 필 4개(Get Noticed·Win Jobs·Work Smarter·Boost Profits) / **테이프 라벨 통계 칩**(92 million+ / 100K+ / 12 hours+ / 44%)이 살짝 기울어진 스티커처럼 붙음 / **손그림 화살표·동그라미 SVG가 UI 카드 사이를 잇는 다이어그램**(AI 섹션 3열) / 전폭 사진 스트립 5장 / 업종 타일 12개(아이콘+라벨) / 인라인 이메일 캡처(흰 입력 + 라임 버튼)가 네이비 밴드에서 2회 반복 / 증언 카드 뒤 거대한 저대비 J 로고 워터마크 / 라임 로고의 푸터 5열.
  - shot-390: 찢어진 종이 경계 유지. **탭 필 행이 좌우 화살표 캐러셀로 변환**(`‹ Get Noticed | Win Jobs ›`)되는 게 모바일 고유 처리. 깨진 리소스 이미지도 동일하게 노출.
- **복합 위젯(기계 기록 불가 → 기술로)**
  - **찢어진 종이 마스크**: SVG/PNG 마스크로 밴드 경계를 만든다. 좌우로 이어지는 비반복 형태라 값이 아니라 자산으로 취급해야 한다.
  - **히어로 사진 모자이크 마퀴**: 기존 분석에 따르면 8장을 4벌씩 복제해 무한 루프를 구성한다(이미지 59개 중 32개만 고유). record.animations는 0건이라 실행 궤적은 잡히지 않았다.
  - **모바일 탭 캐러셀**: 데스크톱의 4개 필 행이 모바일에서 좌우 화살표 캐러셀로 바뀐다. 전환 로직은 기록되지 않았다.
  - 손그림 화살표 다이어그램: UI 카드 3개를 잇는 곡선 화살표가 레이아웃에 종속돼 폭이 바뀌면 다시 그려야 한다.
- **재현 불가 목록** — 실사진 대량(히어로 모자이크 8장·사진 스트립 5장·증언 인물), 찢어진 종이 마스크 자산, 손그림 SVG(화살표·밑줄·동그라미), iframe 8개, jobberPro(자체 폰트), OneTrust 쿠키 배너, 고객 후기 인물·상호, **깨진 리소스 카드 이미지 5장 이상(원본 결함)**.
- **판정: 리드 자격 있음(조건부).** 초기 보류의 첫째 사유였던 **히어로 유실이 해소**됐고(오버레이는 `layers`로 분리), 섹션 10 vs 눈 9 · 모바일 9로 골격이 깨끗하다. 컨테이너 1,360 단일 · 팔레트 2종 · weight 900 지배라는 재료가 또렷하고 hover 9건(28~34프레임)은 5개 중 최상이다. 남은 제약 둘을 안고 가야 한다. **(1) 이미지 rect 결합률 24%** — 슬롯 우선 규칙의 입력(치수·종횡비)이 이미지 4장 중 3장에서 비어 있고, 일부는 원본이 깨져 있어 재수집으로도 못 메운다. 자산 슬롯은 히어로(12개)와 rect가 잡힌 22장 범위에서만 신뢰할 것. **(2) `#6`~`#8`을 리소스 영역 하나로 병합하고 헤드라인 "Our business is helping yours succeed."를 되살릴 것.**
- **계기 동결본 확인(2026-08-17)** — 동결본 재생성에서도 **섹션 10 / 모바일 9 · 컨테이너 [1360] · 헤드라인 없는 섹션 0 · layers 1**로 불변이다.
