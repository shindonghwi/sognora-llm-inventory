...
description: 레퍼런스 웹사이트의 **화면(프론트)** 을 눈대중이 아니라 측정값으로 복제한다. 실제 크롬(Playwright)을 띄워 뷰포트·hover·스크롤 등 상태별로 캡처하고, 요소별 위치·크기·폰트·여백을 수치로 뽑아 명세를 만든 뒤, 구현본을 원본과 동일 조건에서 비교해 오차 게이트를 통과할 때까지 반복한다. 폰트·이미지는 원본 파일을 그대로 받아 쓴다. 트리거 — "이 사이트 똑같이 만들어줘", "레퍼런스 복제", "이 페이지 클론", "디자인 그대로 구현", "원본이랑 비교해서 차이 잡아줘", "픽셀 단위로 맞춰줘", "replicate this site", "clone this page". 비대상 — 새 디자인 창작, 텍스트만 복사, 백엔드·기능 구현(로그인·결제·검색 등 서버 로직)은 이 스킬이 아니다.
---
...
**0) 배치 규칙부터 확정한다 (코드 작성 전).** 파일을 어디에 둘지 스스로 정하지 않는다.
- 대상 프로젝트의 규칙 문서를 먼저 읽는다: `CLAUDE.md`·`AGENTS.md`·`docs/*.md`·`README`. 구조가 문서화돼 있으면 **그대로 따른다**(그 문서가 SSOT다).
- 문서가 없으면 **기존 코드에서 관례를 추론**한다: 라우팅 방식(App Router `src/app/**` vs pages), 페이지 구현이 라우트 파일 안에 있는지 별도 계층(`src/features/{page}/`)에 있는지, 공용 컴포넌트 위치, 정적 자산 위치(`public/`), 스타일 방식(Tailwind·CSS Module·styled).
- 문서도 없고 기존 코드도 비어 있으면(빈 레포·새 프로젝트) **배치안을 선택지로 제안해 확인받는다** — 프레임워크 관례 기본안과 대안을 제시한다. 이것은 §7 무인 완주 예외의 "사용자만 답할 수 있는 것"에 해당한다.
- 확정한 배치 규칙을 `_replica/contract.json`의 `layout` 필드에 적고 **전 페이지에 일관 적용**한다.
